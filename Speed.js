...
!(async () => {
  if($.isTile()) {
    await notify('Speed', 'Panel', 'Starting...')
  }
  ...
  title = `NetSpeed`
  content = `Down: ${round(Math.abs(speed * 8))} Mbps\nPing: ${pingt} ms\nTime: ${round(Math.abs(duration, 2),2)}s`
  if ($.isTile()) {
    await notify('Speed', 'Panel', 'Done')
  } else if(!$.isPanel()) {
    await notify('Speed', title, content)
  }
})()
.catch(async e => {
  $.logErr(e)
